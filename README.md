...
A dashboard on its own doesn't explain itself — here's the thinking behind it.

### What problem was I solving?
Cloud bills grow quietly, and by the time Finance notices, nobody can say why it went up or what to do about it. The real problem isn't "make a Power BI dashboard" — it's that spend is scattered across subscriptions with no owner, nobody has quantified how much is pure waste (idle resources, missing shutdown schedules), and there's no repeatable process to catch a spike before the invoice arrives. This project builds the analysis a FinOps analyst would deliver in their first 30 days to answer that.

### Why did I choose these metrics? 
Each metric maps to a specific decision, not just a number that sounds impressive:

Idle-resource cost over a rolling 90-day window (not a single-day snapshot) — because a resource that's briefly quiet isn't the same problem as one that's been dead for three months; the window filters noise from real waste.
Coefficient of variation on daily usage, not just "is it On-Demand" — because pricing-model spend alone doesn't tell you whether a specific resource is safe to commit to for a year; a spiky workload committed to a reservation just moves the waste, it doesn't remove it.
Standard-deviation-based anomaly detection instead of a fixed dollar threshold — because a fixed number goes stale as the business grows; a rolling trailing-average threshold adapts and gives an alert rule that still works in six months.
Tag-compliance rate by resource group, not just an overall percentage — because "22% of spend is untagged" isn't actionable on its own, but "5 resource groups account for 83% of it" tells someone exactly where to start.

### What did I find — and what decision does it enable? 
See Key Findings and Insights & Recommendations for the full numbers, but the short version: the two most actionable levers found were a non-prod shutdown schedule (the single largest dollar opportunity) and a tagging cleanup concentrated in five resource groups — both decisions a stakeholder can act on this quarter, not abstract "cut costs" advice. Just as importantly, the analysis also found no good case for new Reserved Instance purchases right now — a real recommendation is knowing when not to spend, not just where to cut.
...
build the analysis and reporting layer a FinOps analyst would deliver in their first 30 days — quantify the waste, rank it by impact, and hand leadership a dashboard they can act on.
...
If you're a recruiter or hiring manager and want to talk through the design decisions behind this project, I'd love to
